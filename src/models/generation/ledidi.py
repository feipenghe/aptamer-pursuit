...
class PyTorchRegressor():
    """A wrapper for a PyTorch regression model.

    This wrapper holds a PyTorch model that has regression outputs. The
    ...
    def loss_gradient(self, x, y, mask=None):
        ...
        mask = torch.Tensor(mask / mask.sum())

        x_var = torch.Tensor(x)
        
        with self._model.eval():
            pred_y = self._model(x_var)
    ...
    def loss(self, x, y, mask=None):
        ...
        pred_y = self._model((x.astype('float64')))
